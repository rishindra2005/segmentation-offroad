"""
SegFormer-B2 Training Script V2 - MICRO FOCUS
Refined for fine-grained details: Boundary Loss, RandomResizedCrop, and Edge-Awareness.
"""
...
# ============================================================================
# Dynamic Weight Manager (Refined for Fine-Tuning)
# ============================================================================

class DynamicWeightManager:
    def __init__(self, n_classes, momentum=0.6):
        self.n_classes = n_classes
        self.momentum = momentum
        # More aggressive weights for small features in V2
    ...
    def update_weights(self, val_ious):
        ...
        self.current_weights = torch.from_numpy(np.clip(updated, 0.1, 15.0)).float().cuda()
        print(f"\n[Dynamic Weights Updated] -> {np.round(self.current_weights.cpu().numpy(), 2)}")
        return self.current_weights

# ============================================================================
# Advanced Losses: Weighted Tversky + OHEM + Boundary
# ============================================================================

def boundary_loss(pred, target):
    """Simple edge-aware penalty using Laplacian gradients."""
...
class MicroCombinedLoss(nn.Module):
    def __init__(self):
        super(MicroCombinedLoss, self).__init__()
        self.ohem = OHEMCrossEntropy()
        self.tversky = TverskyLoss()

    def forward(self, logits, targets, weights):
        loss_ohem = self.ohem(logits, targets, weights)
        loss_tversky = self.tversky(logits, targets, weights)
        loss_boundary = boundary_loss(logits, targets)
        # Combination: Focus on edge refinement
        return 0.4 * loss_ohem + 0.4 * loss_tversky + 0.2 * loss_boundary

# ============================================================================
# Dataset: With RandomResizedCrop
# ============================================================================
...
class MaskDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.augment:
            # 1. Random Resized Crop (The MICRO secret)
            ...
            mask = TF.resized_crop(mask, i, j, h, w, size=self.crop_size, interpolation=transforms.InterpolationMode.NEAREST)
            
            # 2. Other joint augments
            if random.random() > 0.5: image, mask = TF.hflip(image), TF.hflip(mask)
            
            # 3. No-Hue Color Jitter
...
def evaluate_metrics_with_tta(model, loader, device, loss_fct, weights):
    ...
    with torch.no_grad():
        for imgs, labels, _ in pbar:
            ...
            logits2 = torch.flip(F.interpolate(model(imgs_flipped).logits, size=imgs.shape[2:], mode="bilinear", align_corners=False), dims=[3])
...
def main():
    ...
    w, h = 952, 532 # Resolution compatible with 14 patches
    batch_size, acc_steps, base_lr = 1, 4, 1e-4 # Lowered LR for fine-tuning
    n_epochs = 15

    script_dir = os.path.dirname(os.path.abspath(__file__))
    output_dir = os.path.join(script_dir, 'train_stats_v2')
    weights_dir = os.path.join(script_dir, 'weights_v2')
    samples_dir = os.path.join(output_dir, 'samples')
    for d in [output_dir, weights_dir, samples_dir]: os.makedirs(d, exist_ok=True)

    # Fixed Transform: Added Resize to ensure image and mask spatial sizes match exactly
    ...
    train_loader = DataLoader(MaskDataset(os.path.join(script_dir, 'Offroad_Segmentation_Training_Dataset/train'), transform, mask_transform, True, crop_size=(h,w)), batch_size=batch_size, shuffle=True, num_workers=4, pin_memory=True)
    ...
    baseline_path = os.path.join(script_dir, "segformer_adv_best.pth")
    if os.path.exists(baseline_path):
        print(f"Loading baseline weights from {baseline_path}...")
        model.load_state_dict(torch.load(baseline_path))
    else:
        print("Warning: Baseline weights not found. Starting from scratch.")

    optimizer = optim.AdamW(model.parameters(), lr=base_lr)
    scheduler = get_cosine_schedule_with_warmup(optimizer, num_warmup_steps=len(train_loader), num_training_steps=len(train_loader)*n_epochs)
    loss_fct = MicroCombinedLoss().to(device)
    ...
    print("\nStarting V2 Fine-Tuning (Macro to Micro)...")
    for epoch in range(n_epochs):
        ...
        print("-" * 50)
        print(f"MEAN {'':<13} | {history['train_iou'][-1]:.3f}    | {history['val_iou'][-1]:.3f}    | {history['val_acc'][-1]:.3f}")
        print("=" * 50)
        ...
        if history['val_iou'][-1] > best_iou:
            ...
            torch.save(model.state_dict(), os.path.join(script_dir, "segformer_v2_best.pth"))
